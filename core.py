...
SYSTEM_PROMPT = (
    ...
    "\n"
    "SCOPE:\n"
    "- Only translate requests that describe an operation or query to perform "
    "on a Linux system (files, processes, storage, logs, networking, packages, "
    "services, users, etc.).\n"
    "- For greetings, small talk, or questions about you or your identity "
    "(e.g. 'who are you'), output exactly: UNSUPPORTED\n"
    "- NEVER use a command such as 'echo' to answer a question, describe "
    "yourself, or converse. A command must PERFORM the requested operation — "
